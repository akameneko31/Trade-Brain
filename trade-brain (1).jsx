...
const ALL_QUESTIONS = [
  ...
  { id: 9, factor: "impulse", text: "エントリー後に根拠を後付けしたことがある。", reverse: false },
...
// Quick check uses these Q IDs
const QUICK_IDS = [1, 3, 5, 7, 9, 10, 13, 17, 21, 24];
...
const FACTOR_META = {
  ...
  loss: {
    ...
    color: "#E63946",
    icon: "💔",
    highDesc: "損失に対する情動反応が強い。含み損がメンタルに影響しやすく、リベンジトレードのリスクがある。",
...
// ════════════════════════════════════════
// Pixel encoding: 0=transparent 1=outline 2=skin 3=white 4=pupil 5=mouth 6=hair 7=primary 8=accent 9=special
...
function shuffleQuestions(questions) {
  // Try full shuffle up to 10 times; pick best result (fewest consecutive violations)
  let bestResult = null;
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    while (pool.length > 0 && stuck < 200) {
      ...
      if (
        len >= 2 &&
        result[len - 1].factor === candidate.factor &&
        result[len - 2].factor === candidate.factor
      ) {
        stuck++;
        continue;
      }
    ...
    // Append any stuck items by inserting at valid positions
    for (const leftover of pool) {
      ...
      for (let i = result.length; i >= 0; i--) {
        ...
        const prevOk = !prev || !prevPrev || !(prev.factor === leftover.factor && prevPrev.factor === leftover.factor);
...
function RadarChart({ scores, size = 300 }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: "100%", maxWidth: size }}>
      ...
      {dataPoints.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="4.5" fill={FACTOR_META[FACTOR_ORDER[i]].color} stroke="#0d1117" strokeWidth="2" />
      ))}
      {labels.map((l) => (
        <text key={l.key} x={l.x} y={l.y} textAnchor="middle" dominantBaseline="central"
          fill="rgba(255,255,255,0.8)" fontSize="10.5" fontWeight="600" fontFamily="'Noto Sans JP', sans-serif">
  ...
  return (
    <div style={{ width: "100%", background: "rgba(255,255,255,0.06)", borderRadius: 8, height: 5, overflow: "hidden" }}>
      <div style={{
        ...
        borderRadius: 8, transition: "width 0.4s cubic-bezier(.4,0,.2,1)",
      }} />
    </div>
  );
}
...
function AnimBar({ score, color, delay = 0 }) {
  ...
  useEffect(() => { const t = setTimeout(() => setW(score), 80 + delay); return () => clearTimeout(t); }, [score, delay]);
  return (
...
function calcFactorScores(answers, questionSet, applyInago) {
  ...
  // Contrast stretch: 2/4回答でもスコアが分散するよう補正
  FACTOR_ORDER.forEach((k) => { scores[k] = contrastStretch(scores[k]); });

  // Clamp all
  FACTOR_ORDER.forEach((k) => { scores[k] = Math.min(100, Math.max(0, scores[k])); });
  return scores;
}

function calcStyleScores(scores) {
  const clamp01 = (x) => Math.min(1, Math.max(0, x));
  const R = scores.reward / 100, L = scores.loss / 100, I = scores.impulse / 100;
  const P = scores.speed / 100, A = scores.analysis / 100, T = scores.time / 100;
  return {
    scalp: Math.round(clamp01(0.35 * R + 0.35 * P + 0.20 * I + 0.10 * (1 - L)) * 100),
    dayTrade: Math.round(clamp01(0.30 * R + 0.30 * P + 0.20 * I + 0.20 * (1 - L)) * 100),
    swing: Math.round(clamp01(0.20 * R + 0.10 * P + 0.25 * A + 0.25 * T + 0.20 * (1 - L)) * 100),
    fundamental: Math.round(clamp01(0.10 * R + 0.10 * P + 0.35 * A + 0.35 * T + 0.10 * (1 - L)) * 100),
    index: Math.round(clamp01(-0.10 * R + 0.05 * P + 0.30 * I + 0.40 * T + 0.20 * (1 - L)) * 100),
  };
}
...
function generateAnalysis(scores, styleScores) {
  ...
  const top1 = sorted[0], top2 = sorted[1], low1 = sorted[5];

  // 高スコア時: 「〜」+「タイプ」で自然につながる形
  const strength = {
    reward: "値動きへの感度が高く、リスクを積極的に取りにいける",
    loss: "損失に対する感受性が鋭く、リスク管理への意識が強い",
...
function getWarnings(styleScores, scores) {
  ...
  if (styleScores.index > 70 && scores.reward < 40)
    w.push("インデックス/積立投資が最も合理的な選択肢です。");
  if (scores.reward > 75 && scores.impulse < 40)
    w.push("衝動制御が課題です。飛びつきエントリーに注意してください。");
  if (scores.loss > 75)
    w.push("損失反応が非常に高い傾向です。ポジションサイズの見直しを推奨します。");
  return w;
}

function shareToX(scores, styleScores, mode) {
  const { primary, secondary } = determineTypes(scores);
  const type = TRADER_TYPES[primary];
  const subType = TRADER_TYPES[secondary];
  const stSorted = Object.entries(styleScores).sort((a, b) => b[1] - a[1]);
  const top1 = stSorted[0], top2 = stSorted[1];
  const top1Name = top1[1] >= 60 ? STYLE_NAMES[top1[0]] : null;
  const top2Name = top2[1] >= 60 ? STYLE_NAMES[top2[0]] : null;

  let styleLine;
  if (top1Name && top2Name) {
    styleLine = `▶ 適性: ${top1Name} / ${top2Name}`;
  } else if (top1Name) {
    styleLine = `▶ 適性: ${top1Name}`;
  } else {
    styleLine = `▶ 適性: バランス型`;
  }

  const lines = [
    `【トレード脳力診断${mode === "quick" ? " 簡易版" : ""}】`,
    ``,
...
export default function TradeBrainDiagnostic() {
  ...
  const startQuiz = useCallback((m) => {
    ...
    let qs;
    if (m === "quick") {
      qs = ALL_QUESTIONS.filter((q) => QUICK_IDS.includes(q.id));
    } else {
      qs = [...ALL_QUESTIONS]; // all 27
    }
  ...
  const goBack = useCallback(() => {
    if (currentIdx > 0 && !animating) {
      setFadeIn(false);
      setTimeout(() => { setCurrentIdx(prev => prev - 1); setFadeIn(true); }, 180);
    }
      ...
      <div style={S.container}>
        <div style={S.card}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <p style={S.desc}>
              あなたの投資行動特性を多角的に分析し、適性のある投資スタイルを判定します。
            </p>

            {/* Mode selection */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
              <button style={S.modeBtn} onClick={() => startQuiz("quick")}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ color: "#e6edf3", fontSize: 16, fontWeight: 700 }}>⚡ 簡易チェック</div>
                    <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 12, marginTop: 4 }}>10問 ・ 約1〜2分</div>
                  </div>
                  <span style={{ color: "#4ECDC4", fontSize: 20 }}>→</span>
                </div>
              </button>

              <button style={{ ...S.modeBtn, border: "1px solid rgba(46,196,182,0.35)", background: "rgba(46,196,182,0.06)" }}
                onClick={() => startQuiz("full")}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ color: "#e6edf3", fontSize: 16, fontWeight: 700 }}>📊 本格測定</div>
                    <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 12, marginTop: 4 }}>27問 ・ 約3〜5分 ・ レーダーチャート付き</div>
                  </div>
                  <span style={{ color: "#4ECDC4", fontSize: 20 }}>→</span>
                </div>
              </button>
      ...
      <div style={S.container}>
        <div style={{
          ...
          <style>{`
            ...
            @keyframes preludeFadeUp2 { from { opacity:0; transform:translateY(18px); } to { opacity:0.7; transform:translateY(0); } }
            @keyframes preludeFadeUp3 { from { opacity:0; transform:translateY(18px); } to { opacity:0.7; transform:translateY(0); } }
            @keyframes preludeBtnFade { from { opacity:0; } to { opacity:1; } }
          `}</style>

          <p style={{
            color: "rgba(255,255,255,0.45)", fontSize: 13, letterSpacing: 2,
          ...
          <p style={{
            ...
            animation: "preludeFadeUp3 0.8s ease 1.2s both",
          }}>
            今の自分をありのまま答えてください
          </p>
          ...
          <button
            style={{
              ...
              letterSpacing: 1,
              animation: "preludeBtnFade 0.6s ease 2.0s both",
            }}
      ...
      <div style={S.container}>
        <div style={S.card}>
          ...
          <div style={{
            ...
            minHeight: 64, marginBottom: 22,
          }}>
            <p style={{ color: "#e6edf3", fontSize: 16, lineHeight: 1.75, fontWeight: 500, margin: 0 }}>
              {q.text}
            </p>
          </div>
          ...
          <div style={{
            ...
            opacity: fadeIn ? 1 : 0, transition: "opacity 0.28s ease",
          }}>
            {LIKERT.map((opt) => {
              ...
              return (
                <button key={opt.value} onClick={() => handleAnswer(opt.value)} style={{
                  ...
                  transition: "all 0.18s", fontFamily: "'Noto Sans JP', sans-serif",
          ...
          {currentIdx > 0 && (
            <button onClick={goBack} style={S.backBtn}>← 前の質問に戻る</button>
          )}
        </div>
      </div>
    );
  ...
  const usedQs = isQuick ? ALL_QUESTIONS.filter((q) => QUICK_IDS.includes(q.id)) : ALL_QUESTIONS;
  const scores = calcFactorScores(answers, usedQs, !isQuick);
  const styleScores = calcStyleScores(scores);
  const stSorted = Object.entries(styleScores).sort((a, b) => b[1] - a[1]);
  const warnings = isQuick ? [] : getWarnings(styleScores, scores);
  const analysisText = isQuick ? null : generateAnalysis(scores, styleScores);
    ...
    <div style={S.container}>
      <div style={{ ...S.card, maxWidth: 480, padding: "32px 20px" }}>
        ...
        <div style={{
          ...
          <p style={{
            color: "rgba(255,255,255,0.35)", fontSize: 12, margin: "0 0 4px",
            letterSpacing: 1,
            animation: "typeFadeIn 0.6s ease both",
          }}>
            {isQuick ? "QUICK CHECK RESULT" : "FULL ANALYSIS RESULT"}
          </p>
          <p style={{
            color: "rgba(255,255,255,0.5)", fontSize: 13, margin: "0 0 20px",
            animation: "typeFadeIn 0.6s ease 0.2s both",
          }}>
            あなたは…
          </p>
          ...
          <div style={{
            ...
            animation: "pixelPop 0.7s cubic-bezier(.2,.8,.3,1.2) 0.5s both",
          }}>
          ...
          {/* Primary Type Name */}
          <p style={{
            color: mainType.color, fontSize: 13, fontWeight: 600,
            margin: "0 0 4px", letterSpacing: 2,
            animation: "typeFadeIn 0.6s ease 0.9s both",
          }}>
            ─── {mainType.title} ───
          </p>
          <h1 style={{
            color: "#e6edf3", fontSize: 28, fontWeight: 800,
            margin: "0 0 4px", fontFamily: "'Noto Sans JP', sans-serif",
            animation: "typeReveal 0.7s cubic-bezier(.2,.8,.3,1) 1.0s both",
          }}>
            {mainType.name}
          </h1>
          <p style={{
            color: "rgba(255,255,255,0.3)", fontSize: 11, margin: "0 0 8px",
            letterSpacing: 1.5, fontFamily: "'JetBrains Mono', monospace",
            animation: "typeFadeIn 0.5s ease 1.1s both",
          }}>
            {mainType.sub}
          </p>
          {/* Attr badge */}
          <div style={{
            ...
            background: `${mainType.color}18`,
            border: `1px solid ${mainType.color}30`,
            marginBottom: 14,
            animation: "typeFadeIn 0.5s ease 1.2s both",
          }}>
            <span style={{ color: mainType.color, fontSize: 12, fontWeight: 700, letterSpacing: 1 }}>
              {mainType.attr}
            </span>
          </div>
          <p style={{
            color: "rgba(255,255,255,0.6)", fontSize: 14, lineHeight: 1.7,
            margin: "0 0 8px", maxWidth: 340, marginLeft: "auto", marginRight: "auto",
            animation: "typeFadeIn 0.6s ease 1.4s both",
          }}>
            {mainType.desc}
          </p>
          <p style={{
            color: "rgba(255,255,255,0.35)", fontSize: 12, margin: "0 0 20px",
            animation: "typeFadeIn 0.5s ease 1.5s both",
          }}>
          ...
          <div style={{
            ...
            borderRadius: 10,
            animation: "typeFadeIn 0.6s ease 1.7s both",
          }}>
          ...
          {elapsedStr && (
            <p style={{ color: "rgba(255,255,255,0.2)", fontSize: 11, margin: "12px 0 0" }}>
              回答時間: {elapsedStr}
            </p>
          )}
        </div>
        ...
        {analysisText && (
          <div style={{
            padding: "16px 20px", background: "rgba(46,196,182,0.04)",
            border: "1px solid rgba(46,196,182,0.12)", borderRadius: 12, marginBottom: 24,
          }}>
            <h2 style={{ color: "#4ECDC4", fontSize: 13, fontWeight: 700, margin: "0 0 8px", letterSpacing: 1 }}>
              💡 あなたの投資行動分析
            </h2>
        ...
        {/* 6 Factor Scores */}
        <div style={S.section}>
          <h2 style={S.secTitle}>6因子スコア{isQuick ? "（簡易）" : ""}</h2>
                ...
                <div key={key}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <span style={{ color: "#e6edf3", fontSize: 14, fontWeight: 600 }}>
                      ...
                      {!isQuick && <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 11, marginLeft: 6 }}>{meta.name}</span>}
                    </span>
                    <span style={{ color: meta.color, fontSize: 18, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>
                      {score}
                    </span>
                  </div>
        ...
        <div style={S.section}>
          <h2 style={S.secTitle}>
            投資スタイル適性{isQuick ? "（参考値）" : ""}
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {(isQuick ? stSorted.slice(0, 3) : stSorted).map(([key, score], rank) => (
              <StyleRow
                key={key}
        ...
        {warnings.length > 0 && (
          <div style={S.section}>
            ...
            {warnings.map((w, i) => (
              <div key={i} style={{
                padding: "12px 16px", background: "rgba(230,57,70,0.05)",
                border: "1px solid rgba(230,57,70,0.15)", borderRadius: 10,
                color: "rgba(255,255,255,0.7)", fontSize: 13, lineHeight: 1.6, marginBottom: 8,
              }}>{w}</div>
            ))}
          </div>
        )}
        ...
        {isQuick && (
          <div style={{
            padding: "20px", background: "rgba(46,196,182,0.04)",
            border: "1px solid rgba(46,196,182,0.15)", borderRadius: 14,
            textAlign: "center", marginBottom: 20,
          }}>
            <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 13, margin: "0 0 12px", lineHeight: 1.6 }}>
              より精密な分析・レーダーチャート・行動リスク判定は本格測定で
            </p>
            <button style={S.ctaBtn} onClick={() => startQuiz("full")}>
              📊 本格測定を受ける（27問）
            </button>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button style={S.shareBtn} onClick={() => shareToX(scores, styleScores, mode)}>
            𝕏 で結果をシェア
          </button>
          <button style={S.retryBtn} onClick={() => { setPhase("intro"); setMode(null); }}>
            もう一度
          </button>
        </div>

        <p style={{ color: "rgba(255,255,255,0.2)", fontSize: 11, textAlign: "center", marginTop: 20, lineHeight: 1.6 }}>
          本診断は投資助言ではありません。結果は自己分析の参考としてご利用ください。
          {!isQuick && <><br />損失反応(1-L)は「反応が低い＝情動安定」として適性計算に使用しています。</>}
          {!isQuick && <><br />イナゴ補正(±5点上限)がReward/Impulseに適用されています。</>}
        </p>
      </div>
    </div>
  );
...
const S = {
  ...
  title: {
    ...
    textAlign: "center", margin: 0, letterSpacing: -0.5,
  },
  subtitle: {
    color: "rgba(255,255,255,0.28)", fontSize: 12,
    textAlign: "center", margin: "4px 0 18px",
    letterSpacing: 2, textTransform: "uppercase",
  },
  desc: {
    color: "rgba(255,255,255,0.55)", fontSize: 14,
    lineHeight: 1.7, textAlign: "center", margin: "0 0 24px",
  },
  modeBtn: {
  ...
  backBtn: {
    ...
    cursor: "pointer", marginTop: 14, padding: "8px 0",
    fontFamily: "'Noto Sans JP', sans-serif",
  },
  section: {
